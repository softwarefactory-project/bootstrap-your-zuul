...
module Config = {
  ...
  module ZuulConfig = {
    @react.component
    let make = (~config: config, ~postHook: RemoteAPI.posthook_t<config, zuul_config>) => {
      // The hook to query the api
      let (zuul_config, post) = postHook

      // Debounch update
      React.useEffect1(() => {
        let handler = Js.Global.setTimeout(() => post(config)->ignore, 500)
  ...
  module Form = {
    ...
    @react.component
    let make = (~postHook: RemoteAPI.posthook_t<config, zuul_config>) => {
...
module Main = (Fetcher: RemoteAPI.HTTPClient) => {
  module Hook = RemoteAPI.Hook(Fetcher)
  @react.component
  let make = () => {
    <Components.Page>
      ...
      <Components.Main>
        ...
        <Components.Container>
          <Config.Form
            postHook={Hook.usePost("/api", Config.config_encode, Config.zuul_config_decode)}
          />
        </Components.Container>
      </Components.Main>
